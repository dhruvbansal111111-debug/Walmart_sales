...
-- Q8 Identify 5 branch and categories with highest decrease ratio in
-- revenue compare to last year (current 2023 and last year 2022)-- 

with revenue_2022 as (select branch,category,sum(total) as revenue_2022
from walmart
where Year(str_to_date(date,'%d/%m/%y')) = 2022
group by branch,category
order by branch),
 revenue_2023 as (select branch,category,sum(total) as revenue_2023
from walmart
where Year(str_to_date(date,'%d/%m/%y')) = 2023
group by branch,category
order by branch)

select ls.branch,ls.category,
